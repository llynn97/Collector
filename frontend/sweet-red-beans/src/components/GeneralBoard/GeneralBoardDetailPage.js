...
const GeneralBoardDetailPage = () => {
    ...
    useEffect(()=>{
        ...
        axios.get('http://localhost:8080/general-board/detail',{
    ...
    //삭제버튼 눌렀을 때
    const deleteConfirm = () => {
        axios.delete('http://localhost:8080/general-board/detail',{
        withCredentials: true,
        data: {
                    post_id : postid,
                }
    ...
    const commentAddClick = () => {
        if(commentContent !== "") {
            ...
            axios.post('http://localhost:8080/general-board/comment', body, { withCredentials: true })
            .then(response => {
                if(response.data.result){
                    console.log("댓글 작성됨");
                    navigation(0)
