...
const SignUp = () => {
    ...
    return (
        ...
        <form className={style.whiteBox}>
            ...
            <div>
                <label className={style.emailText}>
                    이메일<br></br>
                    <input className={style.emailInputText} id="email" type="email" placeholder="example@naver.com" onChange={onEmailChange} value={email} maxLength="100"/>
                    <button className={style.dupChkButton} id="emailCheck" onClick={clickEmailCheck}>중복 확인하기</button>
                </label>
            </div>
            {email === "" ? <div className={style.errormessage} style={{color : 'red'}}>*이메일을 입력해주세요</div> : null}
            {!emailForm && <div className={style.errormessage} style={{color : 'red'}}>*이메일 형식이 아닙니다</div>}
            <br></br>
            <div>
                <label className={style.passwordText}>
                    비밀 번호<br></br>
                    <input className={style.inputText} id="password" type="password" placeholder="********" onChange={onPasswordChange} value={password}/>
                </label>
            </div>
            {!passwordForm && <div className={style.errormessage} style={{color : 'red'}}>*8자 이상, 특수문자, 숫자 포함</div>}
            <br></br>
            <div>
                <label className={style.passwordChkText}>
                비밀번호 확인<br></br>
            ...
            {passwordError && <div className={style.errormessage} style={{color : 'red'}}>*비밀번호가 일치하지 않습니다</div>}
            <br></br>
            <div>
                <label className={style.emailText}>
                닉네임<br></br>
                <input className={style.emailInputText} id="nickname" type="text" placeholder="닉네임" onChange={onChange} value={nickname} maxLength="15"/>
                <button className={style.dupChkButton} id="nicknameCheck" onClick={clickNicknameCheck}>중복 확인하기</button>
                </label>
            </div>
            {nickname === "" ? <div className={style.errormessage} style={{color : 'red'}}>*닉네임을 입력해주세요</div> : null}
            <br></br>
            <input className={style.joinButton} type="submit" value="가입하기" onClick={onClick}/>

            {emailError && <div className={style.errormessage} style={{color : 'red'}}>이메일 중복확인해주세요</div>}
            {nicknameError && <div className={style.errormessage} style={{color : 'red'}}>닉네임 중복확인해주세요</div>}
            {passwordFormError && <div className={style.errormessage} style={{color : 'red'}}>비밀번호 양식을 확인해주세요</div>}
        </form>
        </>
