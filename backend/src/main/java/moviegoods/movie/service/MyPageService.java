...
public class MyPageService {
    ...
    public MyPageResponseSearch search(MyPageRequestSearch mprs){
        ...
        List<Object[]> row= em.createQuery("select c.comment_id , d.content, d.written_date from comment c join c.user u left join c.content_detail d where u.user_id =:user_id order by d.written_date DESC").setParameter("user_id",user_id).getResultList();
        ...
        for (Object[] objects : row) {
            ...
            myPageResponseSearch.getComment().add(new MyPageComment(comment_id,comment_content,written_date));
        ...
        List<Object[]> row2=  em.createQuery("select t.transaction_id , c.content , c.written_date from transaction t join t.content_detail c left join t.user u where u.user_id =:user_id order by c.written_date DESC").setParameter("user_id",user_id).getResultList();
        ...
        for (Object[] objects : row2) {
            ...
            myPageResponseSearch.getWriteTransaction().add(new MyPageTransaction(transaction_id,content,written_date));
        }

        List<Object[]> row3=  em.createQuery("select t.transaction_id from like_basket b join b.user u left join b.transaction t where u.user_id =:user_id ").setParameter("user_id",user_id).getResultList();
        for (Object[] objects : row3) {
            Long transaction_id=(Long)objects[0];
            List<Object[]> row4=  em.createQuery("select c.content, c.written_date from transaction t join t.content_detail c where t.transaction_id =:transaction_id").setParameter("transaction_id",transaction_id).getResultList();
            Object[] result4=row4.get(0);
            String content=(String)result4[0];
            LocalDateTime written_date=(LocalDateTime)result4[1];
            myPageResponseSearch.getLikeTransaction().add(new MyPageTransaction(transaction_id,content,written_date));
        }
