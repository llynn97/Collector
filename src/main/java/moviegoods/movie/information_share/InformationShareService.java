...
public class InformationShareService {
   ...
   // private final InformationShareRepository informationShareRepository;
    ...
    public List<InformationShareResponseSearch> Search(InformationShareRequestSearch isrs){
        ...
        if(!search_word.equals("")){
            ...
            if(sort_name.equals("제목")){
                ...
                searchJpql1=searchJpql1+"where (c.content like '%"+search_word+"%') "+"OR (p.title like '%"+search_word+"%')";
                ...
                for (Post post : postList) {
                    ...
                    Content_detail content_detail=informationShareContent_detailRepository.findById(content_detail_id).get();
        ...
        for (Post post : postList) {
            ...
            Long views=post.getViews();
            Long post_id=post.getPost_id();
            LocalDateTime written_date=informationShareContent_detailRepository.findById(post.getContent_detail().getContent_detail_id()).get().getWritten_date();
            searchList.add(makeInformationShareResponseSearch(nickname,title,views,post_id,written_date));
    ...
    public InformationShareResponseDetail detailInfo(InformationShareRequestDetail isrd){
        ...
        //String jpql="select p From post p join p.content_detail c where p.post_id="+post_id;
       ...
       List<Object[]> row=em.createQuery("select u.user_id, u.nickname, p.views,p.title,p.image_url,d.written_date," +
        ...
        List<Object[]> row2=em.createQuery("select u.user_id, u.nickname, d.content, d.written_date from comment c join c.post p left join c.user u left join c.content_detail d where p.post_id=:post_id").setParameter("post_id",post_id).getResultList();
        List<Comments> result2=new ArrayList<>();
        for (Object[] objects : row2) {
            Boolean check=false;
            Comments comment=new Comments((Long)objects[0],(String)objects[1],(String)objects[2],(LocalDateTime)objects[3]);
